/*Ejemplo 2A: ne ete ejemplo la tarea emisora usa la API Receive y la tara receptora la API Peek, como los datos leidos de la cola
nunca se elimina la cola se llane. Esto permite tener dos opciones: que la tarea emisroa se bloquee o que esta realice otra accion*/
#include <stdio.h>
...
void task_emisora(void *params)
{
    ...
    while(true)
    {
        if (xQueueSend(cola, &contador, pdMS_TO_TICKS(5)) == pdPASS)
        {
...
void task_receptora(void *params) 
{
    ...
    while(true) 
    {   
        xQueuePeek(cola, &dato_recibido, portMAX_DELAY);
...
int main(void) 
{
    ...
    xTaskCreate(task_emisora, "Emisor", 256, NULL, 1, NULL);
